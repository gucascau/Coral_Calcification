...
##################################################
my $Version     = "1.0";
my $ReleaseDate = "January 11, 2012";
my $Author      = "Zhang Zhang";
my $Email       = "zhangzhang\@big.ac.cn";
my $Function    = "Parallel Alignment & Translation";
my $Reference   = "Zhang Z., et al. ParaAT: a parallel tool for constructing multiple protein-coding DNA alignments, under review.";
...
if ( $ProcessFile eq "" || !(-e $ProcessFile) ) {
...
print "\t Process file = $ProcessFile\n";
...
sub AlignProteinSeqs(){
	...
	if ($MSACMD =~ /clustalw2/) {
	    $cmd = "clustalw2 -INFILE=".$file.".pep  -PWMATRIX=BLOSUM -TYPE=PROTEIN -OUTPUT=FASTA -OUTFILE=".$file.".pep_aln >> msg.msa";
	}
	elsif ($MSACMD =~ /mafft/) {
	    $cmd = 	"mafft  --auto --quiet  ".$file.".pep  >  ".$file.".pep_aln";
	}
	elsif ($MSACMD =~ /muscle/) {
        ...
        $cmd = 	"t_coffee  ".$file.".pep  -output=fasta  -outfile=".$file.".pep_aln -quiet >> msg.msa";
	}
	return $cmd;
}
